...
class SyncUserPlayerAndBalance
{
    ...
    private function createPlayerToken($user)
    {
        ...
        try {
            $result = ApiTransactions::createUser($user->username);

            if ($result['success'] ?? false) {
                $user->player_token = $result['data']['username'];
                $user->save();
                return true;
            }
            ...
            Log::warning("API createUser gagal (tapi tidak lempar exception)", [
                'user_id' => $user->id,
                'player_token' => $user->username,
                'response' => $result,
            ]);
        } catch (\Throwable $e) {
            Log::warning("Create user API gagal", [
                'user_id' => $user->id,
                'player_token' => $user->username,
                'error' => $e->getMessage(),
            ]);
    ...
    {
        try {
            $getBalance = ApiTransactions::checkBalance($user->player_token);
